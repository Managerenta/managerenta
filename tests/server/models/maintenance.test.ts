...
describe("getMaintenanceRequestsDB", () => {
	it("lists only the user's non-deleted requests, newest first", async () => {
		...
		await MaintenanceRequest.create(
			payload({ title: "Foreign", userId: OTHER }),
		);

		const { requests, total } = await getMaintenanceRequestsDB({
			userId: USER,
		});
		expect(total).toBe(2);
	...
	it("filters by status, priority, propertyId, and treats 'all' as no filter", async () => {
		...
		const byStatus = await getMaintenanceRequestsDB({
			userId: USER,
			status: "completed",
		});
		expect(byStatus.requests.map((r) => r.title)).toEqual(["B"]);

		const byPriority = await getMaintenanceRequestsDB({
			userId: USER,
			priority: "low",
		});
		expect(byPriority.requests.map((r) => r.title)).toEqual(["A"]);

		const byProperty = await getMaintenanceRequestsDB({
			userId: USER,
			propertyId: "p2",
		});
	...
	it("searches title and description case-insensitively", async () => {
		...
		const res = await getMaintenanceRequestsDB({
			userId: USER,
			search: "heater",
		});
	...
	it("treats regex metacharacters literally and does not throw", async () => {
		...
		const wild = await getMaintenanceRequestsDB({
			userId: USER,
			search: "$^|.*+?",
		});
	...
	it("paginates with limit/offset and caps limit at MAX_LIMIT (50)", async () => {
		...
		const page = await getMaintenanceRequestsDB({
			userId: USER,
			limit: 2,
			offset: 1,
		});
		...
		const capped = await getMaintenanceRequestsDB({
			userId: USER,
			limit: 999,
		});
	...
	it("presigns image keys through the post-aggregate hook", async () => {
		await createMaintenanceRequestDB({
			payload: payload({
				images: [
					`maint-${crypto.randomUUID()}.jpg`,
					`maint-${crypto.randomUUID()}.png`,
				],
			}) as never,
		});
...
describe("getMaintenanceRequestByIdDB", () => {
	it("returns the request for its owner", async () => {
		const req = await createMaintenanceRequestDB({ payload: payload() });
		const found = await getMaintenanceRequestByIdDB({
			id: req!.id,
			userId: USER,
		});
...
describe("deleteMaintenanceRequestDB", () => {
	...
	it("returns false for wrong owner or malformed id", async () => {
		const req = await createMaintenanceRequestDB({ payload: payload() });
		expect(
			await deleteMaintenanceRequestDB({ id: req!.id, userId: OTHER }),
		).toBe(false);
		expect(
			await deleteMaintenanceRequestDB({ id: "nope", userId: USER }),
		).toBe(false);
	});
});
...
describe("getMaintenanceStatsDB", () => {
	it("counts requests per status for the user", async () => {
		...
		await MaintenanceRequest.create(
			payload({ title: "2", status: "open" }),
		);
		await MaintenanceRequest.create(
			payload({ title: "3", status: "in-progress" }),
		);
		await MaintenanceRequest.create(
			payload({ title: "4", status: "completed" }),
		);
		await MaintenanceRequest.create(
			payload({ title: "5", status: "cancelled" }),
		);

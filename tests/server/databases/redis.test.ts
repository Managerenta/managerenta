...
describe("redisUpdateKeyString / redisRetrieveKeyString", () => {
	it("stores JSON with a default 60s TTL and reads it back typed", async () => {
		...
		expect(await redisUpdateKeyString(key("default-ttl"), value)).toBe(true);

		const ttl = await Redis.ttl(key("default-ttl"));
		expect(ttl).toBeGreaterThan(0);
		expect(ttl).toBeLessThanOrEqual(60);

		const roundTrip = await redisRetrieveKeyString<typeof value>(key("default-ttl"));
		expect(roundTrip).toEqual(value);
	});

	it("honours a custom TTL in seconds", async () => {
		expect(await redisUpdateKeyString(key("custom-ttl"), "v", true, 120)).toBe(true);
	...
	it("persists without expiry when expire=false", async () => {
		expect(await redisUpdateKeyString(key("no-ttl"), [1, 2, 3], false)).toBe(true);
		expect(await Redis.ttl(key("no-ttl"))).toBe(-1); // -1: exists, no TTL
		expect(await redisRetrieveKeyString<number[]>(key("no-ttl"))).toEqual([1, 2, 3]);
	});
	...
	it("actually expires: a 1s key is gone after its TTL", async () => {
		expect(await redisUpdateKeyString(key("short"), "gone-soon", true, 1)).toBe(true);
		expect(await redisRetrieveKeyString<string>(key("short"))).toBe("gone-soon");
